...
const AdministrationForm = ()=>{
    ...
    useEffect(()=>{
        const request = async()=>{
            const fetching = await fetch(`/categories`)            
    ...
    useEffect(()=>{
        const request = async()=>{
            const fetching = await fetch(`/cities`)            
            const response = await fetching.json()  
            setCities(response)    
        }
        request()
    ...
    useEffect(()=>{
        const request = async()=>{
            const fetching = await fetch('/features')
            const response = await fetching.json()
            dispatch({
            ...
            case 200:
                ...
                const postFeatures = async()=>{
                    for(const feature of features){
                        const request  = await fetch(`/products/${idProduct}/link/${feature?.id}`,{
                            method: 'PATCH',
                            headers
                          })
